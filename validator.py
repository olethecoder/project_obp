...
class UnitTest():
    ...
    def run_all_tests(self):
        """Run all tests in sequence and return overall pass/fail status"""
        try:
            # Run all tests in sequence
            self.same_end_day()
            self.shifts_coverage_matrix()
            self.tasks_coverage_matrix()
            
            # Get results from validation checks
            coverage_result = self.check_coverage_sufficiency()
            solution_times_result, solution_times_message = self.check_solution_times()
            
            # Overall test result
            all_tests_passed = coverage_result and solution_times_result
            
            print("\n=== Test Summary ===")
            print(f"Coverage Test: {'✓' if coverage_result else '✗'}")
            print(f"Solution Times Test: {'✓' if solution_times_result else '✗'}")
...
def run_tests(shifts_df, tasks_df):
    """Main function to run all unit tests"""
    tester = UnitTest(shifts_df, tasks_df)
    return tester.run_all_tests()

if __name__ == "__main__":
    # Example dataframes would go here
    shifts_df = pd.read_csv('data/shifts_solution_Ole.csv') # Your shifts data
    tasks_df = pd.read_csv('data/tasks_solution_Ole.csv')   # Your tasks data
    
    # Run all tests with one line
    success = run_tests(shifts_df, tasks_df)