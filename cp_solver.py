...
class OptimalNurseSchedulerCP:
    ...
    def __init__(
        ...
        """Initialize the CP solver with preprocessed data.

        Args:
            shift_info (list): Each element is a dict describing one shift.
            shift_start_blocks (dict): block -> list of shift indices that start at this block.
            tasks_info (list): Each element is a dict describing a day-specific task.
            task_map (list of tuple): (original_task_idx, day_index) for each tasks_info entry.
            min_nurses_anytime (int, optional): A global minimum number of nurses. Defaults to 0.
            max_solve_time (float, optional): Time limit for solver in seconds. Defaults to 60.0.
        """
    ...
    def _build_model(self):
        ...
        #    h[b] = 1 if starts_at[b] >= 1
        starts_at = [0]*N_BLOCKS
        max_possible_usage_per_block = [0]*N_BLOCKS

        # Precompute maximum usage for each block
        ...
        # Build linear expressions for starts_at[b]
        for b in range(N_BLOCKS):
            if b in self.shift_start_blocks:
        ...
        # Create a binary variable h[b] to indicate if any nurse starts at block b
        h = []
        for b in range(N_BLOCKS):
            ...
            if M > 0:
                ...
                # If no shift starts here, h[b] = 0
                self.model.Add(hb == 0)

        # 4) COVERAGE constraints:
        #    effective_coverage(b) = sum(shift_usage[s]) - starts_at[b] - h[b]
        #    must be >= tasks_demand(b) AND >= min_nurses_anytime
        for b in range(N_BLOCKS):
            coverage_terms = []
            for s_idx, sh in enumerate(self.shift_info):
                if sh["coverage"][b] > 0:
    ...
    def solve(self):
        """Solve the model, capturing intermediate solutions, and return results.

        Returns:
            tuple:
                total_cost (float): The final solution cost (scaled down).
                shift_usages (list of int): The assigned usage for each shift.
                tasks_solution (list of dict): Each day-specific task with
                    solution start time, etc.
                intermediate_solutions (list of (float, float)):
                    Solutions found during search (objective, time_elapsed).
        """
        ...
        # Extract final objective cost
        total_cost_scaled = solver.ObjectiveValue()
        total_cost = total_cost_scaled / 100.0

        # SHIFT usage
        shift_usages = [solver.Value(var) for var in self.shift_usage_vars]
        ...
        for i, tinfo in enumerate(self.tasks_info):
            ...
            orig_task_idx, day_index = self.task_map[i]  # Use the map
